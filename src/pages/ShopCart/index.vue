...
export default {
	...
	computed: {
		...
		totalPrice() {
			return this.cartInfoList.reduce((pre, cartInfo) => {
				if (cartInfo.isChecked) {
					//统计选中的商品价格总和
					pre += cartInfo.skuNum * cartInfo.skuPrice
				}
				return pre
			}, 0)
		},
	...
	methods: {
		...
		async changeChecked(index) {
			...
			this.cartInfoList[index].isChecked = this.cartInfoList[index].isChecked ? 0 : 1
			try {
				await this.$store.dispatch('changeChecked', { skuId: this.cartInfoList[index].skuId, isChecked: this.cartInfoList[index].isChecked })
				this.getData()
			}
			catch (error) {
				alert('修改商品选择状态失败')
