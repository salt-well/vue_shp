<template>
  <div class="clearfix selector">
    <div class="type-wrap logo">
      ...
      <div class="ext">
        <a href="javascript:void(0);" class="sui-btn">更多<i>﹀</i></a>
        <a href="javascript:void(0);" class="sui-btn"><i>+</i>多选</a>
      </div>
    </div>
      ...
      <div class="fl value">
        <ul class="type-list">
          <li v-for="(attrValue, index) in attr.attrValueList" :key="index">
            <a @click="addAttrValue(attr.attrId,attr.attrName,attrValue)">{{ attrValue }}</a>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    ...
    addTrademark(tmId,tmName) {
      //子组件调用父组件事件传参
      this.$emit('addTrademarkParams', tmId, tmName)
    },
    //添加属性面包屑
    addAttrValue(attrId,attrName,attrValue){
      this.$emit('addAttrValueParams',attrId,attrName,attrValue)
    }
